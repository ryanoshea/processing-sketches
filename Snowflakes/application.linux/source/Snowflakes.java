...
float fractionSingleBranchPoints = 0.6f;
...
public void drawGreeting() {
  ...
  if(time <= startFade) {
    ...
    alpha = 255 - 200.0f*(time - startFade)/(endFade-startFade);
    g1 = 150.0f + 105.0f*(time - startFade)/(endFade-startFade);
    b1 = 150.0f + 105.0f*(time - startFade)/(endFade-startFade);
    r2 = 150.0f + 105.0f*(time - startFade)/(endFade-startFade);
    b2 = 150.0f + 105.0f*(time - startFade)/(endFade-startFade);
  }
...
public float animationPhase(float periodSec, float offset) {
  ...
  return (TWO_PI/periodSec*millis()/1000.0f + offset) % TWO_PI;
}
...
class SnowflakeManager {
  ...
  public void spawnSnowflake(boolean firstSpawning) {
...
class Snowflake {
  ...
  float angularSpeed;
  float branchFraction = random(0.2f, 0.8f); // + 0.2*sin(animationPhase(8,0));
  float lengthFraction = random(0.4f, 0.6f);
  float angle = random(PI/4, PI/2.5f); // + PI/12.0*sin(animationPhase(4,0));
  float myColor;
  int numBranchPoints;
  ...
  public void descend() {
    float age = (millis() - birthTime)/1000.0f;
  ...
  public boolean isBelowScreen() {
    return center.y > (screenY + 1.5f*lengthMainBranches);
  }
  ...
  public void drawBranch(Vector start, Vector end, int recursionDepth, int maxRecursionDepth) {
  ...
  public void drawSubbranch(Vector start, Vector end, float branchFraction, float lengthFraction, float angle, int recursionDepth, int maxRecursionDepth) {
...
class Vector {
  ...
  public Vector plus(float xoff, float yoff) {
  ...
  public Vector interpolate(Vector vec, float fraction) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--hide-stop", "Snowflakes" });
  }
}
